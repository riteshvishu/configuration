...
enhancement PolicyPeriodBaseEnhancement : PolicyPeriod {
  ...
  public property get PolicyNumberAssigned(): boolean {
    return Plugins.get(IPolicyPeriodPlugin).UnassignedPolicyNumberIdentifier != this.PolicyNumber
  }
  ...
  function checkBaseEditability() : boolean {
    ...
    if (job == null) {
      Logger.logDebug("checkBaseEditability() - FALSE: NULL Job")
      return false
    }

    // Cannot edit a policy period that has unhandled preemptions
    if (this.hasAnyUnhandledPreemptions()) {
      Logger.logDebug("checkBaseEditability() - FALSE: Has unhandles preemptions")
      return false
    }

    // Unprivileged users cannot edit a PolicyPeriod under UW EditLock
    if (UWLockedAndNoOverride) {
      Logger.logDebug("checkBaseEditability() - FALSE: UWLocked and No Override")
      return false
    }
    ...
    if (wrongSlice and not this.FailedOOSEValidation) {
      Logger.logDebug("checkBaseEditability() - FALSE: wrong slice and not a Failed OOS validation")
      return false
    }
    return true
  }
  ...
  function addLocations(locations: List<AccountLocation>) {
    for (loc in locations) {
      ...
      if (this.WorkersCompLineExists and this.WorkersCompLine.getJurisdiction(JurisdictionMappingUtil.getJurisdictionMappingForAccountLocation(loc)) == null) {
        this.WorkersCompLine.addJurisdiction(JurisdictionMappingUtil.getJurisdictionMappingForAccountLocation(loc))
      }
    }
  }
  ...
  function getUWCompanyRateFactor(ratingEffectiveDate : Date, state : Jurisdiction) : Number {
    ...
    if (not queryIterator.hasNext()) {
      ...
      Logger.logWarning(displaykey.PolicyPeriod.UWCompany.NoLicensedStates(comp.DisplayName, product, state, ratingEffectiveDate))
      priceFactor = 1
    } else {
  ...
  */
  ...
  property get JobCompletionMessage() : String {
    switch(this.Status) {
      ...
      case "Canceling"    : return displaykey.Web.JobComplete.Message.ScheduledCancellation(this.Job.JobNumber, this.CancellationDate.toLocaleString())
  ...
  private property get LocationStateSet() : Set<Jurisdiction> {
    ...
    for(loc in this.PolicyLocations) {
      if(loc.State != null) {
        jurisdictions.add(JurisdictionMappingUtil.getJurisdictionMappingForPolicyLocation(loc))
      }
    }
  ...
  function setPeriodDates(effectiveDate : Date) {
    ...
    if (this.TermType == "Other") {
      ...
      var expirationDate = gw.api.util.DateUtil.mergeDateAndTime(
        policyPeriodPlugin.calculatePeriodEnd(effectiveDate, this.TermType),
        this.PeriodEnd)
