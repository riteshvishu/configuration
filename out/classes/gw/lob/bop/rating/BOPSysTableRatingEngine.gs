...
class BOPSysTableRatingEngine extends AbstractRatingEngine<BOPLine> {
  ...
  override protected function createCostDataForCost(c : Cost) : CostData {
    switch(typeof c) {
      case BOPBuildingCovCost:  return new BOPBuildingCovCostData(c)
      case BOPCovBuildingCost:  return new BOPCovBuildingCostData(c)
      case BOPCovCost:          return new BOPCovCostData(c)
      case BOPLocationCovCost:  return new BOPLocationCovCostData(c)
      case BOPMoneySecCovCost:  return new BOPMoneySecCovCostData(c)
  ...
  private function rateBOPEmpDisCov(cov : BOPEmpDisCov) {
    ...
    var cost = new BOPCovCostData(start, end, cov.Currency, cov.BOPLine.BaseState, cov.FixedId)
  ...
  private function rateBOPLiabilityCov(cov : BOPLiabilityCov) {
    ...
    for ( building in cov.BOPLine.BOPLocations*.Buildings ) {
      var cost = new BOPCovBuildingCostData(start, end, cov.Currency, JurisdictionMappingUtil.getJurisdictionMappingForPolicyLocation(building.BOPLocation.Location), cov.FixedId, building.FixedId)
      cost.NumDaysInRatedTerm = this.NumDaysInCoverageRatedTerm
  ...
  private function rateBOPMoneySecCov( cov : BOPMoneySecCov, onPremises : boolean ) {
    var start = cov.SliceDate
    var end = getNextSliceDateAfter(start)
    var cost = new BOPMoneySecCovCostData(start, end, cov.Currency, JurisdictionMappingUtil.getJurisdictionMappingForPolicyLocation(cov.BOPLocation.Location), cov.FixedId, onPremises)
    cost.NumDaysInRatedTerm = this.NumDaysInCoverageRatedTerm
  ...
  private function rateBOPOutSignCov(cov : BOPOutSignCov) {
    var start = cov.SliceDate
    var end = getNextSliceDateAfter(start)
    var cost = new BOPLocationCovCostData(start, end, cov.Currency, JurisdictionMappingUtil.getJurisdictionMappingForPolicyLocation(cov.BOPLocation.Location), cov.FixedId)
    cost.NumDaysInRatedTerm = this.NumDaysInCoverageRatedTerm
    cost.Basis   = 1
    //get rating date for this coverage  and BOPLocation
    //var ratingDate = ReferenceDatePlugin.getCoverageReferenceDate(cov.Pattern as CoveragePattern, cov.BOPLocation)
  ...
  private function rateBOPBuildingCov(cov : BOPBuildingCov) {
    var start = cov.SliceDate
    var end = getNextSliceDateAfter(start)
    var cost = new BOPBuildingCovCostData(start, end, cov.Currency, JurisdictionMappingUtil.getJurisdictionMappingForPolicyLocation(cov.BOPBuilding.BOPLocation.Location), cov.FixedId)
    cost.NumDaysInRatedTerm = this.NumDaysInCoverageRatedTerm
  ...
  private function rateAddlInsFlatCharges(lineVersion : BOPLine) {
    if ( lineVersion.Branch.RefundCalcMethod != "flat" )  // context.CancelMethod returns null if the period is not canceled
    {
      for ( additionalInsured in getRepresentativeAdditionalInsureds(lineVersion) )
      {
        ...
        var cost = new BOPAddnlInsuredCostData(lineVersion.Branch.PeriodStart, lineVersion.Branch.PeriodEnd, lineVersion.PreferredCoverageCurrency, lineVersion.BaseState, additionalInsured.FixedId)
  ...
  private function rateMinimumPremium(lineVersion : BOPLine) {
    if( lineVersion.Branch.RefundCalcMethod != "flat" ) { // context.CancelMethod returns null if the period is not canceled
      ...
      if ( subtotal < minPremium ) {
        var cost = new BOPMinPremiumCostData(lineVersion.Branch.PeriodStart, lineVersion.Branch.PeriodEnd, lineVersion.Branch.PreferredSettlementCurrency, lineVersion.BaseState)
        cost.NumDaysInRatedTerm = lineVersion.Branch.NumDaysInPeriod
        cost.Basis              = subtotal
  ...
  private function rateTaxes(lineVersion : BOPLine) {
    var taxBases = CostDatas
      ...
      .mapValues( \ costs -> costs.sum(\c -> c.ActualAmount))
  ...
  {
    var cost = new BOPTaxCostData(lineVersion.Branch.PeriodStart, lineVersion.Branch.PeriodEnd, TaxRatingCurrency, state)
    cost.NumDaysInRatedTerm = lineVersion.Branch.NumDaysInPeriod
    cost.Basis              = basis
