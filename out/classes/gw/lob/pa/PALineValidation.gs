...
class PALineValidation extends PolicyLineValidation<entity.PersonalAutoLine> {
   ...
   * If policy line's expiration date is after policy effective date plus one year then an error will be issued
  */
  function policyPeriodOneYearMax() {
    if (paLine.ExpirationDate.after(paLine.EffectiveDate.addYears(1))) {
