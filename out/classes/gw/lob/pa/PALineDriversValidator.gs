...
class PALineDriversValidator extends PolicyLineValidation<entity.PersonalAutoLine> {
  ...
  function primaryAddressRequiredFields() {
    paLine.PolicyDrivers.each(\ pd -> {
      ...
      var contact = pd.AccountContactRole.AccountContact.Contact
      var address = contact.PrimaryAddress
      if (!address.AddressLine1.HasContent) {
        Result.addError(paLine, "default", displaykey.Web.Policy.PA.Validation.Address.AddressLine1, DRIVERS_WIZARD_STEP)
      }
      if (!address.City.HasContent) {
        Result.addError(paLine, "default", displaykey.Web.Policy.PA.Validation.Address.City, DRIVERS_WIZARD_STEP)
      }
      if (!address.State.Code.HasContent) {
        Result.addError(paLine, "default", displaykey.Web.Policy.PA.Validation.Address.State, DRIVERS_WIZARD_STEP)
      }
      if (!address.CityStateZip.HasContent) {
        Result.addError(paLine, "default", displaykey.Web.Policy.PA.Validation.Address.ZIP, DRIVERS_WIZARD_STEP)
      }
      if (!address.AddressType.Code.HasContent) {
  ...
  private function verifyAndOrderMVRs() {
    Logger.logDebug("begin verifyAndOrderMVRs")
    if(Level == ValidationLevel.TC_BINDABLE or Level == ValidationLevel.TC_READYFORISSUE){
      ...
      Logger.logDebug("Number of mvrWorkflows: " + paLine.Branch.Workflows.whereTypeIs(ProcessMVRsWF).length)
      if(mvrWorkflow <> null){
        var synchronousWait = true
        Logger.logDebug("begin initiateMVRRequest")
        mvrWorkflow.initiateMVRRequest(synchronousWait)            
        Logger.logDebug("done initiateMVRRequest")   
    ...
    Logger.logDebug("done verifyAndOrderMVRs")
  }
