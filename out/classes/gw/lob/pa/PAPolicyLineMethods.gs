...
class PAPolicyLineMethods extends AbstractPolicyLineMethodsImpl {
  ...
  override function getCostForCoverage(covered : Coverable, covPat : CoveragePattern) : Cost {
    return _line.Branch.AllCosts.firstWhere(\ cost -> {
      return cost typeis PACost and
             ...
             cost.Coverage.Pattern.Code.equals(covPat.Code) and
  ...
  override function onBeginIssueJob() {
    ...
    Logger.logDebug("Before cancelling wfs in PAPolicyLineMethods")
    period.Workflows.whereTypeIs(ProcessMVRsWF).where(\ w -> w.State == WorkflowState.TC_ACTIVE).each(\ w -> {w.cancelFromOutsideWF(); w.Bundle.commit()})
    Logger.logDebug("After cancelling wfs in PAPolicyLineMethods")
        ...
        garagedVehicles.each(\ vehicle -> {
          ...
          vehicleCosts
            ...
            .each(\ cost -> {
              ...
              while (costWithWorksheet.PARatingWorksheet == null and costWithWorksheet.BasedOn != null) {
