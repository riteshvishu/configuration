...
@Export
class WC7RatingEngine extends WC7AbstractRatingEngine<productmodel.WC7Line> implements WC7BasicTemplateRatingEngine {

  var _calculator = new WC7RatingProcessorCalculator()
  ...
  var _rateFactorSearch : WC7RateFactorSearch as readonly RateFactorSearch
   ...
   */
  override property get Payroll() : AutoMap<Jurisdiction, BigDecimal> { return _payroll }

  construct(line : WC7Line, minRatingLevel : RateBookStatus) {
    super(line, minRatingLevel)
    MinimumPremium = 0
    _rateFactorSearch = new WC7RateFactorSearch(line.Branch, minRatingLevel)
  }
  ...
  /**
   * Calculate the manual rates for all of the normal WC7CoveredEmployee (location / class code) exposure units
   */
  private function rateCoveredEmployees()
  {
    ...
    for ( emp in allWC7CoveredEmployees )
    {
      addCost(WC7CoveredEmployeeRater.rate( this, emp ))
    }
  ...
  function storeRatingSubtotal(amount : BigDecimal, subtotal : RateSubtotalType, granularity : WC7RatingSubtotalGranularity, st : Jurisdiction, rpsd : String) {
    _calculator.storeRatingSubtotal( amount, subtotal, granularity, st, rpsd )
  }

  override function getRatingSubtotal(subtotal : RateSubtotalType, granularity : WC7RatingSubtotalGranularity, st : Jurisdiction, rpsd : String) : BigDecimal {
   ...
   */
  override function convertRateByUsage( rate : BigDecimal, usage : RateConversionType ) : BigDecimal {
    switch ( usage )
    {
   ...
   */
  override function updateMinimumPremium( theMinPremium : BigDecimal, rateState : Jurisdiction, normalizedClassCode : String ) {
  ...
  function processWCRatingSteps(fromStep : int, toStep : int, atRatingPeriodLevel : boolean ) {
    ...
    {
      var ratingPeriods = atRatingPeriodLevel ? juris.RatingPeriods : new ArrayList<WC7RatingPeriod>(){ new WC7RatingPeriod( juris, juris.Branch.PeriodStart, juris.Branch.PeriodEnd, 1 ) }
   ...
   */
  private function processWCRatingStepsByPeriod( ratingPeriod : WC7RatingPeriod, fromStep : int, toStep : int, atRatingPeriodLevel : boolean )
    {
    ...
    var queriedSteps = new WC7RatingStepsSearchCriteria( ratingPeriod.RatingDate, ratingPeriod.Jurisdiction.Jurisdiction ).matchAllInRange( fromStep, toStep )
    ...
    // job
    var steps : List<WC7RatingStepExt>
    if (PolicyLine.Branch.Job typeis Audit && PolicyLine.Branch.Audit.AuditInformation.AuditScheduleType == "PremiumReport") {
    ...
    for (step in steps) {
      ...
      switch( step.stepAction )
      {
        case "subtotal":
          ...
          break
        case "modifier":
          addCost(WC7ModifierRater.rate( ratingPeriod, step, atRatingPeriodLevel, this ))
          break
        case "fee":
          break
        case "custom":
          ...
          {
            case "emp_liab":
              addCost(WC7EmployersLiabilityIncreasedLimitsRater.rate(ratingPeriod, step, atRatingPeriodLevel, this ))
              break
            case "min_prem":
              addCost(WC7PolicyLevelMinimumPremiumRater.rate(ratingPeriod, step, this))
              break
            case "exp_constant":
              addCost(WC7ExpenseConstantRater.rate(ratingPeriod, step, this))
              break
            case "prem_discount":
              addCost(WC7PremiumDiscountRater.rate(calcBlendedPremiumDiscountRate( ratingPeriod, step ), ratingPeriod, step, this))
              break
            case "terrorism":
              addCost(WC7TerrorismPremiumRater.rate(this, ratingPeriod, step, this))
              break
  ...
  private function storeSubtotal(ratingPeriod : WC7RatingPeriod, step : WC7RatingStepExt, atRatingPeriodLevel : boolean) {
    var amount = getCostAmountSum(ratingPeriod)
    PCFinancialsLogger.logDebug("Subtotal: " + step.subtotal + " Level: " + (atRatingPeriodLevel ? "ratingPeriod" : "jurisdiction") + " State: " + ratingPeriod.Jurisdiction.Jurisdiction.Code + " Date: " + ratingPeriod.RatingStart + " Amount: " + amount)
    storeRatingSubtotal( amount, step.subtotal, atRatingPeriodLevel ? WC7RatingSubtotalGranularity.ratingPeriod : WC7RatingSubtotalGranularity.jurisdiction, ratingPeriod.Jurisdiction.Jurisdiction, ratingPeriod.RatingStart as String )
  }
  
  //private function rateShortRatePremium( ratingPeriod : WC7RatingPeriod, step : WC7RatingStepExt )
  //{
  //  if ( PolicyLine.Branch.RefundCalcMethod == "shortrate" )
  //  {
  //    var shortRateFactor = new WC7ShortRateFactorSearchCriteria( ratingPeriod.RatingDate, PolicyLine.Branch ).match( ratingPeriod.Jurisdiction.Jurisdiction )
  //    addCost(WC7ShortRatePremiumRater.rate(shortRateFactor.shortRateFactor, ratingPeriod, step, this))
  //  }
  //}

  override function getOrCalcSubtotal( ratingPeriod : WC7RatingPeriod, step : WC7RatingStepExt, atRatingPeriodLevel : boolean ) : BigDecimal {
    ...
    {
      basis = getRatingSubtotal( step.subtotal, atRatingPeriodLevel ? WC7RatingSubtotalGranularity.ratingPeriod : WC7RatingSubtotalGranularity.jurisdiction, ratingPeriod.Jurisdiction.Jurisdiction, ratingPeriod.RatingStart as String )  // fetch the subtotal
    }
    return basis
  }
   ...
   */
  private function calcBlendedPremiumDiscountRate( ratingPeriod : WC7RatingPeriod, step : WC7RatingStepExt ) : BigDecimal
  {
    if (PolicyLine.Branch.Job typeis Audit && PolicyLine.Branch.Audit.AuditInformation.IsPremiumReport) {
       var existingCost = ratingPeriod.getExistingWC7JurisdictionCost( step )
      ...
      var blendedRate = 0 as BigDecimal
      ...
      {
        var factor = RateFactorSearch.getPremiumDiscountFactor(policyStandardPrem, ratingPeriod.RatingDate)
        var accumDiscount = 0 as BigDecimal
        accumDiscount = accumDiscount + (policyStandardPrem * factor)
...
//        accumDiscount = accumDiscount + ( premiumInRange * factor.factor )
  ...
  override property get RoundingLevel() : int
  {
    return PolicyLine.Branch.Policy.Product.QuoteRoundingLevel
  }

  override property get RoundingMode() : RoundingMode {
    return PolicyLine.Branch.Policy.Product.QuoteRoundingMode
  }

  override function getCostAmountSum(ratingPeriod : WC7RatingPeriod) : BigDecimal {
    return CostDatas.where(\c -> matchesPeriod(c, ratingPeriod))
      .sum( \ c -> c.ActualAmount )
  }

  private function matchesPeriod(cost : CostData, ratingPeriod : WC7RatingPeriod) : boolean {
    var start = ratingPeriod.RatingStart.trimToMidnight()
    var end = ratingPeriod.RatingEnd.trimToMidnight()
    if (cost typeis WC7JurisdictionCostData) {
      var stateCost = cost
      return stateCost.EffectiveDate.trimToMidnight() >= start && stateCost.ExpirationDate.trimToMidnight() <= end && stateCost.State == ratingPeriod.Jurisdiction.Jurisdiction
    } else {
      var empCost = cost as WC7CovEmpCostData
      return empCost.EffectiveDate.trimToMidnight() >= start && empCost.ExpirationDate.trimToMidnight() <= end && empCost.State == ratingPeriod.Jurisdiction.Jurisdiction
    }
  }
  ...
  {
    if ( PolicyLine.Branch.RefundCalcMethod != "flat" )
    {
      // Prorate the "minimum premium to meet" in all cases except for short-rate cancellations.
      var numDaysToProrate = PolicyLine.Branch.RefundCalcMethod == "shortrate" ? PolicyLine.Branch.NumDaysInPeriod : PolicyLine.Branch.NumDaysInUncanceledPeriod
      var numDaysInStandardPeriod = PolicyLine.Branch.NumDaysInStandardPeriod
      if ( numDaysInStandardPeriod == null )
      {
        ...
        numDaysInStandardPeriod = p.financialDaysBetween( PolicyLine.Branch.PeriodStart, PolicyLine.Branch.PeriodStart.addYears( 1 ) )
      }
  ...
  private function determineLargestExpenseConstant( )
  {
    ...
    for ( juris in PolicyLine.RepresentativeJurisdictions )
    {
      var expConst = RateFactorSearch.getExpenseConstantFactor()
      if ( expConst > ExpenseConstant
           or ( expConst == ExpenseConstant and ExpenseConstantState > juris.Jurisdiction ) )
      {
        ExpenseConstant      = expConst
        ExpenseConstantState = juris.Jurisdiction
      }
    }
  }

  override property get WC7Line() : WC7Line {
    return PolicyLine
  }
