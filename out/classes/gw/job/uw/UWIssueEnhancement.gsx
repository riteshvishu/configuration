...
enhancement UWIssueEnhancement : UWIssue {
  ...
  property set Approval(arg : UWIssueApproval) {
    if (arg.Issue != this) {
      throw new IllegalStateException("Should only be 'assigning' UWIssueApproval to non-owning issue")
    }
    if (arg == null) {
      // make sure the associated approval information is cleared out
   ...
   *
   * <p>This method should be called exactly once whean a new issue is created or re-created,
