...
class SubmissionProcess extends NewTermProcess {
  ...
  override function bind() {
    ...
    TransactionUtil.runAtomically(\ -> startBinding(), _branch.Bundle)
  }
  ...
  private function startBinding() {
    // make sure no other producer can lay claim to this policy
    if (not _branch.isRiskReserved()) {
      ...
      JobProcessLogger.logInfo("Declining branch because it is risk reserved: " + _branch)

      PCProfilerTag.BIND_DECLINE.execute(\ -> {
        _branch.addNote(TC_RISK, displaykey.Submission.RiskReserved.Note.Subject,
                        displaykey.Submission.RiskReserved.Note.Body(_branch.Policy.Product, _branch.Policy.Account))
        declineJob()
      })
    }
  }
  ...
  private function finalizeBindIssue() {
    ...
    _branch.promoteBranch(false) // commits...

    // commit changes since branch promotion...
    //_branch.Bundle.commit() // already done at the end of TransactionUtil.runAtomically
  }
