...
class AccountHolderPolicyMetrics {
   ...
   * the specified role
   */
  function sumInForcePremiumFor(contact : Contact, roles : Set<Type<AccountContactRole>>, currency : Currency) : MonetaryAmount {
    return activePoliciesQueryFor(contact, roles).select()
      .sumDifferentCurriences(currency, \ policy -> {
   ...
   * the specified role.
   */
  function calculateLifetimePremium(contact : Contact, roles : Set<Type<AccountContactRole>>, currency : Currency) : MonetaryAmount {
    ...
    var sumsOfPriorPremiums = new HashMap<Currency, BigDecimal>()
    activePoliciesQueryFor(contact, roles).select(\ p ->
        new Pair<Currency, BigDecimal>(p.PriorPremiums_cur, DBFunction.Sum(p.PriorPremiums_amt)))
      .reduce<Map<Currency, BigDecimal>>(sumsOfPriorPremiums, \ v, h -> {v.put(h.First, h.Second); return v} )
    ...
    var sumsOfTotalPremiumRPT = new HashMap<Currency, BigDecimal>()
    activePolicyPeriodsQueryFor(contact, roles).select(\ period ->
      ...
      .reduce<Map<Currency, BigDecimal>>(sumsOfTotalPremiumRPT, \ v, h -> {v.put(h.First, h.Second); return v} )

    sumHolder.add(sumsOfTotalPremiumRPT)

    return sumHolder.sum(currency)
  }
