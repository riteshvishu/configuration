...
class MissingCovTermIssueWrapper extends ProductModelSyncIssueWrapper<gw.api.web.productmodel.MissingCovTermIssue> {
  ...
  override property get BaseMessage() : String {
    if (Issue.Fixed) {
      return displaykey.Web.JobWizard.ProductModelSync.MissingCovTermAdded(Issue.Pattern.DisplayName)
    } else {
      return displaykey.Web.JobWizard.ProductModelSync.MissingCovTermNotAdded(Issue.Pattern.DisplayName) 
    }  
  }
