...
class StandardPhoneOwner extends PhoneOwnerBase {
  ...
  override function isVisible(fieldId : PhoneOwnerFieldId) : boolean {
    if (isLocationEditable()){
      return isFieldVisibleEditMode(fieldId)
    }else{
      return isFieldVisibleReadOnlyMode(fieldId)
    }

  }

  protected function isLocationEditable() : boolean{
    return gw.api.util.LocationUtil.CurrentLocationEditable
  }

  protected function isFieldVisibleReadOnlyMode(fieldId : PhoneOwnerFieldId) : boolean {
    return !PhoneOwnerFieldId.HIDDEN_FIELDS_READONLY.contains(fieldId)
  }

  protected function isFieldVisibleEditMode(fieldId : PhoneOwnerFieldId) : boolean {
