...
uses gw.webservice.pc.pc700.productmodel.LookupRootImpl
uses java.util.Date
...
class ProductModelAPI {
  ...
  function getAvailableQuestions(lookupRoot : LookupRootImpl,
    ...
    var product = Product.getByCode(lookupRoot.ProductCode)
    if(product == null){
      throw new BadIdentifierException("Could not find product with code: ${lookupRoot.ProductCode}")
    }
    var coverableType = lookupRoot.lookupType()
    if(coverableType == null){
      throw new BadIdentifierException("Could not find entity: ${lookupRoot.LookupTypeName}")
    }
  ...
  function getAvailableClausePatterns(lookupRoot : LookupRootImpl,
    ...
    SOAPUtil.require(lookupDate, "lookupDate")
    var linePattern = PolicyLinePattern.getByCode(lookupRoot.PolicyLinePatternCode)
    if(linePattern == null){
      throw new BadIdentifierException("Could not find policy line pattern with code: ${lookupRoot.PolicyLinePatternCode}")
    }
    var coverableType = lookupRoot.lookupType()
    if(coverableType == null){
      throw new BadIdentifierException("Could not find entity: ${lookupRoot.LookupTypeName}")
