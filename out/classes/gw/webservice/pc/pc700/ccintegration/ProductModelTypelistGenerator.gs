...
class ProductModelTypelistGenerator {
  ...
  private function genCCProductModelXML() : XMLNode {
    ...
    // each locale folder is named with its language code. ex: fr_CA or en_US
    ...
    for (prod in gw.api.productmodel.Product.getAll() ) {
    ...
    for (lob in LOBProductMap.Keys) {
      for (cov in lob.CoverageCategories.flatMap(\ cat -> cat.CoveragePatterns) ) {
        ...
        for (covTerm in cov.CovTerms) {
          ...
          // Save the localized strings for the coverage
          localizeNameAndDescription(covTerm as ProductModelObjectBase, "CovTermPattern", CovTermPattern.NAME, CovTermPattern.DESCRIPTION)
  ...
  private function localizeNameAndDescription(prodModel : ProductModelObjectBase, typeListName : String,
