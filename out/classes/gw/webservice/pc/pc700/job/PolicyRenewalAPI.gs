...
class PolicyRenewalAPI {
  ...
  function confirmTerm(policyNumber : String, termNumber : int, transactionId : String) {
    SOAPUtil.tryCatch<String>(\ bundle -> {
      ...
      if (query.Empty) {
        throw new BadIdentifierException("Could not find a term with policy number: ${policyNumber} and term number: ${termNumber}.")
      }
      var period = query.FirstResult
      if (period.PolicyTerm.Archived) {
        throw new BadIdentifierException("Policy term ${period.PolicyTerm} is archived")
      }
  ...
  function notifyPaymentReceivedForRenewalOffer(offerID : String,
    ...
    return SOAPUtil.tryCatch<String>(\ bundle -> {
      ...
      if (job == null or not (job typeis Renewal)) {
        throw new BadIdentifierException("Could not find a renewal offer with offerid of ${offerID}")
      }
      ...
      var paymentPlan = getApplicablePaymentPlan(period, selectedPaymentPlanCode, paymentAmount)
      if (paymentPlan == null) {
      ...
      try {
        ...
        period.PaymentPlanID = paymentPlan.BillingId
  ...
  private function getApplicablePaymentPlan(period : PolicyPeriod,
      selectedPaymentPlanCode : String, paymentAmount : BigDecimal) : PaymentPlanSummary{
    var paymentPlan : PaymentPlanSummary
    Transaction.runWithNewBundle(\ bundle -> {
      period = bundle.add(period)
      if (selectedPaymentPlanCode == null) {
        var currentPaymentPlan = period.PaymentPlans.getById(period.PaymentPlanID)
        if (currentPaymentPlan.DownPayment.Amount <= paymentAmount) {
          paymentPlan = currentPaymentPlan
        }
        else if(period.PaymentPlans <> null){
          paymentPlan = period.PaymentPlans
            .where(\ p -> p.DownPayment.Amount <= paymentAmount)
            .maxBy(\ p -> p.DownPayment.Amount)
        }
      }
      else {
        paymentPlan = period.PaymentPlans
          .firstWhere(\ p -> p.BillingId == selectedPaymentPlanCode
            and p.DownPayment.Amount <= paymentAmount)
      }
    })
    return paymentPlan
  }
  ...
  private function startRenewal(accountNumber : String,
    ...
    if(account == null){
      throw new BadIdentifierException("Could not find account with number: ${accountNumber}")
    }
    var product = Product.getByCode(productCode)
    if(product == null){
      throw new BadIdentifierException("Could not find product with code: ${productCode}")
    }
    var producerCode = gw.api.database.Query.make(ProducerCode).compare("PublicID", Equals, producerCodeId).select().AtMostOneRow
    if(producerCode == null){
      throw new BadIdentifierException("Could not find producer code with public id: ${producerCodeId}")
    }
  ...
  private function parseDateFromString(dateString : String, parameterName : String) : Date {
    try {
      ...
      iae.printStackTrace()
      throw new DataConversionException("Cannot parse parameter name: \"${parameterName}\" parameter value: \"${dateString}\" as Date.")
    }
  }
}
