...
gw.DocumentUtil = (function() {
  ...
  function retrieveDocumentContent(url, targetHiddenFrame, alertDownloadFailure) {
    if (targetHiddenFrame) {
      ...
      window.open(url, _ids.documentContentWindow,
          "height=400,width=800,location=0,menubar=0,status=0,toolbar=0,resizable=1,directories=0", true);
    }
  }
  ...
  function sendContentsToDocumentControlAndCacheLocation(docResponse) {
    if (docResponse["errorMsg"]) {
      ...
      withDocumentAssistant(function(documentAssistant) {
        ...
        if (baseSpanName) {
          showEditActionLinks(baseSpanName)
        }
      });
    }
  }

  function getLinkElement(baseSpanName, type) {
    return Ext.fly(baseSpanName + type).parent();
  }

  function showBaseActionLinks(baseSpanName) {
    getLinkElement(baseSpanName, "view").setVisible(true, Ext.Element.DISPLAY);
    getLinkElement(baseSpanName, "edit").setVisible(true, Ext.Element.DISPLAY);
    getLinkElement(baseSpanName, "localEdit").setVisible(false, Ext.Element.DISPLAY);
    getLinkElement(baseSpanName, "upload").setVisible(false, Ext.Element.DISPLAY);
    getLinkElement(baseSpanName, "discard").setVisible(false, Ext.Element.DISPLAY);
  }

  function showEditActionLinks(baseSpanName) {
    getLinkElement(baseSpanName, "view").setVisible(false, Ext.Element.DISPLAY);
    getLinkElement(baseSpanName, "edit").setVisible(false, Ext.Element.DISPLAY);
    getLinkElement(baseSpanName, "localEdit").setVisible(true, Ext.Element.DISPLAY);
    getLinkElement(baseSpanName, "upload").setVisible(true, Ext.Element.DISPLAY);
    getLinkElement(baseSpanName, "discard").setVisible(true, Ext.Element.DISPLAY);
  }
    ...
    var rtn = withDocumentAssistant(function(documentAssistant) {
      try {
        var responseText = documentAssistant.uploadFileContents(fileLocation, 'fileContent',
            gw.app.getConfig().UploadDocumentContentURL, [ 'widgetId', widgetId ]);
        if (processRawUploadResponse(gw.app.localize('Document.Assistant.Error.Banner'), responseText)) {
            if (baseSpanName) {
                showBaseActionLinks(baseSpanName)
            }
        }
      } catch (e) {
  ...
  return {
    ...
    uploadFile : function(documentAssistant, controlID, tempFilePath, commitCurrentLocation) {
      var url = gw.app.getConfig().FileUploadURL;
    ...
    docAction : function(opName, widgetId, baseSpanName) {
      ...
      switch (opName) {
        ...
        case 'editRepositoryVersion' :
          sendAjaxRequest(widgetId, 'displayDocument', {baseSpanName:baseSpanName}, sendContentsToDocumentControlAndCacheLocation);
          return;
        ...
        case 'uploadLocalVersion' :
          sendAjaxRequest(widgetId, 'uploadChangedDocument', {'baseSpanName':baseSpanName}, uploadLocalDocument);
          return;

        case 'discardLocalVersion' :
          sendAjaxRequest(widgetId, 'clearCachedDocumentLocation', {'baseSpanName':baseSpanName}, cleanupFile);
          return;
