...
Selenium.prototype.doSetCellValue = function(gridId, arg) {
  ...
  var waitForCellEditCompletion = eventWin.Test.setCellValue(gridId, rowOffset, dataIndex, value, {
  ...
  // TODO code review CL 417491: E:PLReady:Web: I would rather have a callback function sent to setCellValue that calls
  // an error method that in turn throws the visible SeleniumError exception. I do not like to do outside in
  // type checks of error return codes. It is very brittle and prone to refactoring errors and missing of
  // cases when new return codes/types are added.
  // Another option is to make exceptions like SeleniumError visible for the test packages.
  // Example for callback code bit down here. Place method next to onCheckbox inside setCellValue callback function:
  /*
  onError: function(errorMessage) {
    throw new SeleniumError (errorMessage);
  }
  */
  if (typeof waitForCellEditCompletion == typeof "") {
    throw new SeleniumError (waitForCellEditCompletion); // an error occurred
  }
  ...
  return function() {
    var waitForCompletion = self.doWaitForAlertPresentOrGwOperationDone();
    if (!waitForCompletion) {
      waitForCompletion = function() {return true};
    }
    /**
     ...
     */
    var cellEditStatus = waitForCellEditCompletion();
    if (eventWin.Ext.isString(cellEditStatus)) {
      throw new SeleniumError("Set cell value error: " + cellEditStatus);
    }
    return selenium.isAlertPresent() || cellEditStatus && waitForCompletion();
  };
}
...
function SeleniumGW_clickGw(locator, bReturnPageName) {
  ...
  if (eventWin.Ext.fly(elem).hasCls('g-accordion-item')) {
    selenium.doMouseDown(locator); // menu link in left nav
