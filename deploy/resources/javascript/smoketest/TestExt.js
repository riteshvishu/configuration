...
var Test = function() {
  ...
  function getRowIndex(grid, rowOffset) {
    ...
    return gw.ext.Util.getRowIdxForRowOffset(grid, rowOffset);
  }
  ...
  function getGridCoordinate(grid, rowOffset, dataIndex) {
    ...
    colIndex = gw.ext.Util.getColumnIndexByDataIndex(grid, dataIndex);
    ...
    rowIndex = grid.store.findBy(function(r) {
        var offset = gw.ext.Util.getRowOffsetFromRow(r);
        var noModeOffset = gw.ext.Util.getModeInsensitiveRowOffset(offset);
        if(noModeOffset === rowOffset){
            //Search the column match in gwCellId
            columnId = gw.ext.Util.getColumnIdForCellId(r, dataIndex);
            ...
            Ext.each(elementsToSearch, function(input){
                if(input && input.id){
                    ...
                    if(inputID.indexOf(dataIndex, inputID.length - dataIndex.length) !== -1){
                        colIndex = gw.ext.Util.getColumnIndexByDataIndex(grid, key);
                        dvItem = input;
                        // Append the text if this is a radiogroup.  huh?
                        gw.ext.Util.processGridEditor(grid.getStore(), rowIndex, gw.ext.Util.getColumnIndexByDataIndex(grid, key), function(editorByRow) {
                            if (editorByRow[0].xtype == 'radiogroup') {
                                dvItem.text = gw.ext.Util.getTextForRadioGroupCell(editorByRow[0], dvItem.value);
                            }
                        });
                        return false;
                    }
  ...
  function getNumCellIteratorEntries(store, record, cellIterFullId) {
    ...
    record.fields.eachKey(function(field) {
      var cellId = gw.ext.Util.getFullIdForCell(store, record, field);
   ...
   */
  function getGridById(lvId, rowOffset, dataIndex) {
    // Assume the lvId is for an LV naming id. A LV id with an explicit id has the same render id and component id.
    // If the LV id is constructed for an anonymous LV, then the matching grid needs to be looked for.
    var grid = gw.ext.Util.getGridById(lvId);

    if (!grid) {
      var getMatch = function(component) {
        component = gw.ext.Util.getGridById(component);
        if (component) {
          var gridInfo = gw.ext.Util.getGridInfo(lvId, rowOffset, dataIndex, component);
          component = (gridInfo ? gridInfo.getGrid() : null);
        }
        return component;
      };
      
      
      Ext.ComponentManager.each(function(componentId, component) {
          if (getMatch(component)) {
            grid = component;
            return false;
          }
        });
  ...
  function isMenuParentsDisabled(menuItem) {
    if (menuItem instanceof gw.gmenuitem || menuItem instanceof Ext.menu.Menu) {
...
//            eventObject.initKeyboardEvent(event.substring(2), true, true, null, false, alt, shift, false, keycode, 0);
    ...
    getCellIteratorNumEntries : function(lvId, rowOffset, cellIteratorOffset) {
      var grid = Ext.isString(lvId) ? getGridById(lvId, rowOffset, null) : lvId;
      var count = 0;
    ...
    getLVNumEntries : function(lvId, nestedIteratorOffset) {
      ...
      var grid = Ext.isString(lvId) ?
              getGridById(lvId, nestedIteratorOffset ? nestedIteratorOffset + ":0" : "0", null) :
              lvId;
      var count = 0;

      if (grid && !grid.getStore().isDestroyed) {
        grid.getStore().each(function(record) {
          var rId = record.get(gw.ext.Util.getRowOffsetProperty());

          if (nestedIteratorOffset) {
            rId = gw.ext.Util.getModeInsensitiveRowOffset(rId);
    ...
    getLVTotalRecordsByLVPagingId: function(lvPagingId) {
      var count = 0;
    ...
    getLVTotalPages: function(lvPagingId) {
      var count = 0;
     ...
     */
    getNumEntries : function (iteratorId) {
      // TODO tpollinger test whether this works in LV, can be ambiguous
      var grid = getGridById(iteratorId);
      if (grid != null) {
        return this.getLVNumEntries(grid);
      }
    ...
    isRowSelected : function(lvId, rowOffset) {
      var grid = getGridById(lvId, rowOffset, null);
    ...
    getColumnWidth: function(gridId, dataIndex){
      var grid = getGridById(gridId, 0, dataIndex);
      var column = gw.ext.Util.getColumnById(grid, dataIndex);
      return column.width;
    },

    setColumnWidth: function(gridId, dataIndex, widthValue){
      var grid = getGridById(gridId, 0, dataIndex);
      var colIndex = gw.ext.Util.getColumnIndexByDataIndex(grid, dataIndex);
  ...
  /**
    ...
    getCellValue : function(lvId, rowOffset, dataIndex, prop) {
      ...
      var grid = getGridById(lvId, rowOffset, dataIndex);
      ...
      gw.ext.Util.processGridEditor(grid.store, rowOffset, column, function(editorByRow) {
      ...
      if (Ext.isArray(fieldValue)) {
        return gw.ext.Util.getFirstInputInTemplateCell(fieldValue).value;
      }

      if (prop) {
        return fieldValue[prop];
      }
      return fieldValue.editValue != null ? fieldValue.editValue : fieldValue.text === undefined ?
    ...
    isActiveCellEditor: function(gridId, rowOffset, dataIndex) {
      var isActive = false;
      var grid = getGridById(gridId, rowOffset, dataIndex);
    ...
    getGridRowClass : function(lvId, rowOffset) {
      var grid = getGridById(lvId, rowOffset);
      if (!grid) {
        return null;
      }
    ...
    setCellValue : function(lvId, rowOffset, dataIndex, value, callback) {
      var grid = getGridById(lvId, rowOffset, dataIndex);
      var accord = getGridCoordinate(grid, rowOffset, dataIndex);
      ...
      gw.ext.Util.processGridEditor(grid.store, rowIndex, column, function(editorByRow) {
    ...
    getCellOptionStore : function(lvId, rowOffset, dataIndex) {
      ...
      var col = grid.getView().headerCt.getHeaderAtIndex(accord[1]);  //@SenchaEnhancement headerCt

      var bEditor = false;
      var store = [];
      gw.ext.Util.processGridEditor(grid.store, rowIndex, col, function(editorByRow) {
        bEditor = true;
    ...
    unlockHeader : function(lvId, dataIndex) {
      var grid = getGridById(lvId, null, dataIndex);
    ...
    isHeaderCellVisible : function(lvId, dataIndex) {
      // AHK - 5/22/2013 - getGridById seems to fail for row trees, but just Ext.ComponentManager.get() seems to work fine
    ...
    getColumnFilter: function(grid, dataIndex) {
      ...
      var filter = null;
      if (grid.filters && grid.filters.filters) {
        filter = grid.filters.filters.getByKey(dataIndex);   //TODO: Refactor: Card 370: Column filters as data model
      }
    ...
    showColumnFilterMenuByGrid : function(grid, dataIndex) {
      ...
      var colIdx = gw.ext.Util.getColumnIndexByDataIndex(grid, dataIndex);
    ...
    getColumnFooterElem : function (lvId, rowOffset, dataIndex) {
      ...
      var fqRowOffset = gw.ext.Util.getFQRowOffset(rowOffset);
      fqRowOffset.offset = (fqRowOffset.offset ? fqRowOffset.offset + ":0" : "0");
      var rowOffsetGroup = fqRowOffset.getRepr();
      var colIdx = -1;
      var footerColumnId = null;

      var grid = getGridById(lvId, rowOffsetGroup, dataIndex);
      if (!grid && grid.store && grid.store.proxy) {
        return null;
      }

      var summaryData = grid.store.proxy.reader.jsonData. origSummaryData;
      if (summaryData) {
     //     var footerCellId = grid.id + ":" + dataIndex;
      ...
      if (footerColumnId) {
        colIdx = getColumnIndexForColumnId(grid, footerColumnId);
      }
      ...
      // next footer row
      function getFooterDiv(enclosingOrPrecedingDiv) {
    ...
    isClickable:function (e, compId) {
      ...
      if (comp) { // check the ExtJs component, if any
        ...
        if (comp instanceof gw.gmenuitem) {
        ...
        if (e) { // check the HTML element
          ...
          return !e.disabled &&
               (e.tagName == 'INPUT' || e.tagName == 'TEXTAREA' || e.tagName == 'SELECT' || e.href != null || e.onclick != null ||
                    ...
                    Ext.fly(e).getStyle('color') == 'rgb(0, 103, 172)' || //clickable links are in blue
                 ...
                 //TODO: ExtJs upgrade4: check if a tree node is disabled
                    ...
                    Ext.fly(e).hasCls('x-grid-cell-inner') && Ext.fly(e).first() != null && Ext.fly(e).first().getStyle('color') == 'rgb(0, 103, 172)' // tree grid column
                    );
        } else {
