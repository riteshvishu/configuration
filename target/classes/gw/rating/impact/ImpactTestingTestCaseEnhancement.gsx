...
enhancement ImpactTestingTestCaseEnhancement : entity.ImpactTestingTestCase {
  ...
  function populatePeriods(searchResult : IQueryBeanResult<PolicyPeriod>){
    ...
    if (this.Periods.Count > 0) {
      throw new DisplayableException("There are already periods in this test case")  
    }
  ...
  function reset(){
    ...
    RateFlowLogger.get().info("before removing test periods ")
    
    this.Periods.each(\ i -> {i.refresh(); this.removeFromPeriods(i)})
  ...
  static function getSingletonTestCase() : ImpactTestingTestCase{
    ...
    gw.transaction.Transaction.runWithNewBundle(\ bundle -> {        
      ...
      if(rs.Count > 1){
        throw new DisplayableException("There should only be one test case in the database") 
      }
