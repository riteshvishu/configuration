...
class ReinsuranceConfigPlugin implements IReinsuranceConfigPlugin {
  ...
  var _logger = Logger.forCategory("IReinsuranceConfigPlugin")
  ...
  override function getReinsuranceCurrency(coverages : List <Coverage>) : Currency {
    ...
    if (jurisdiction == null) {
      throw new UserDisplayableException("Cannot find jurisdiction associated with coverables " + coverables)
    }
    var currency = RegionCurrencyMappingUtil.getCurrencyMappingForJurisdiction(jurisdiction)
    if (currency == null) {
      throw new UserDisplayableException("Currency not defined for jurisdiction " + jurisdiction)
    }
    return currency
  }
}