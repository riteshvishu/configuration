...
enhancement FormPatternEnhancement : entity.FormPattern {
  ...
  property get Products() : Product[] {
    return this.FormPatternProducts.map(\ f -> PCDependencies.getProductModel().getPattern(f.ProductCode, Product))
  }
  ...
  property get AvailableProducts() : Product[] {
    ...
    return PCDependencies.getProductModel().getAllInstances(Product).where(\ p -> not addedProductCodes.contains(p.Code)).toTypedArray()
  }
  ...
  function getGenericInferenceClassRange(policyLine : PolicyLinePattern) : FormInferenceClassRef[] {
    ...
    if (policyLine != null) { // apply filter if specified
      ...
      refsList = refsList.where(\ ref -> ref.ValidPolicyLines.containsAll(PolicyLinePattern.getAll()))
    }
  ...
  /**
   * @return an unsorted array of all {@link gw.api.productmodel.PolicyLinePattern} POJOs that
   ...
   * is invalid, the results will still contain a reference for it.
   */
  property get PolicyLinePatternRange() : PatternRef<PolicyLinePattern>[] {
    var lines = (this.Products.HasElements) ? PCDependencies.getProductModel().getAllInstances(PolicyLinePattern) : new ArrayList<PolicyLinePattern>()
    this.Products.each(\ p -> {lines = lines.intersect(p.LinePatterns.toSet())})
  ...
  function loadPolicyLinePatternRef(code : String) : PatternRef<PolicyLinePattern> {
    var pattern = gw.api.productmodel.PolicyLinePattern.getByCode(code)
...
*/
  ...
  property get InferenceClass() : Type<FormData> {
    ...
    return TypeSystem.getByFullName(className) as Type<FormData>
  }
