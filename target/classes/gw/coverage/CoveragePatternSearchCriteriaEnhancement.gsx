...
enhancement CoveragePatternSearchCriteriaEnhancement : gw.api.productmodel.CoveragePatternSearchCriteria {
  ...
  function performClausePatternSearch(linePattern : PolicyLinePattern) : ClausePattern[] {
    var clauseType : Type<? extends ClausePattern> = CoveragePattern
    ...
    if(linePattern == null){
      allClausePatterns = PCDependencies.getProductModel().getAllInstances(clauseType).toTypedArray()
    } else {
