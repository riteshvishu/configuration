...
class ImpactTestingSearchUIHelper {
  ...
  function availableProducts() : Product[]{
    return Product.getAll().subtract(SearchCriteria.Products.toList()).toTypedArray()
  }
  ...
  static function clearWorkQueue(processType : BatchProcessType) {
    ...
    gw.transaction.Transaction.runWithNewBundle( \ bundle -> {
      for (row in rows) {
        var workItem = bundle.loadByKey(row.ID) as StandardWorkItem
        RateFlowLogger.get().debug("removing workitem ${workItem.PublicID} ${workItem.status.DisplayName} ${workItem.DisplayName}")
        workItem.remove()
      }
    })
  }
