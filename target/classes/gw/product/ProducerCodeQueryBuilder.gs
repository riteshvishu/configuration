...
class ProducerCodeQueryBuilder extends EntityQueryBuilder<ProducerCode> {
  ...
  override function doRestrictQuery(selectQueryBuilder : ISelectQueryBuilder) {
    ...
    } else if (_secure && !SecureProducerSearchUtil.addSecurityRestrictions(User.util.CurrentUser, selectQueryBuilder)) {
    ...
    if (_currency != null) {
      var currencyJoin = selectQueryBuilder.subselect("ID", CompareIn, ProducerCodeCurrency, "ProducerCode")
