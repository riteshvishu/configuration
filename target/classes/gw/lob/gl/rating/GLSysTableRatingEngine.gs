...
class GLSysTableRatingEngine extends AbstractRatingEngine<GLLine> {
  ...
  override protected function createCostDataForCost(c : Cost) : CostData {
    switch(typeof c) {
      case GLCovExposureCost:  return new GLCovExposureCostData(c)
  ...
  private function rateAddlInsured(lineVersion : GeneralLiabilityLine) {
    ...
    for (addlInsured in getRepresentativeAdditionalInsureds(PolicyLine)) {
      var cost = new GLAddlInsuredCostData(start, end, lineVersion.Branch.PreferredCoverageCurrency, PolicyLine.BaseState, addlInsured.FixedId)
  ...
  private function rateGLCoverageAndExposureForSublineAndSplitType(cov : GLCovWrapper, exp: GLExposure,
    ...
    var cost = new GLCovExposureCostData(start, end, cov.Cov.Currency, JurisdictionMappingUtil.getJurisdictionMappingForPolicyLocation(exp.Location ), cov.Cov.FixedId, exp.FixedId, exp.ClassCode.Basis.Auditable)
    cost.Subline = subline
    cost.SplitType = splitTypeHandler.SplitType
    cost.NumDaysInRatedTerm = this.NumDaysInCoverageRatedTerm
    ...
    cost.ActualBaseRate = splitTypeHandler.getRateFor(exp.ClassCode.Code, JurisdictionMappingUtil.getJurisdictionMappingForPolicyLocation(exp.Location), subline, start)
    cost.ActualAdjRate = cost.ActualBaseRate
