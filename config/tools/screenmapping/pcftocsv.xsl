...
<xsl:apply-templates select="Item" />
  ...
  <!-- You can add or re-order the elements in the list here to affect the CSV output.  Note that all values are
       ...
       MappingReflect
  -->
<xsl:template match="Item"><xsl:if test="DisplayKey != ''">"<xsl:value-of select="Title" />","<xsl:value-of select="DisplayKey" />","<xsl:value-of select="./Label" />","<xsl:value-of select="ValuePath" />","<xsl:value-of select="./DBTable" />","<xsl:value-of select="FieldName" />","<xsl:value-of select="./ADTFieldTypeShort" />","<xsl:value-of select="ADTFieldLength"/>","<xsl:value-of select="./ADTFieldTypeList" />","<xsl:value-of select="Editable" />","<xsl:value-of select="Required" />","<xsl:value-of select="Visible" />","<xsl:value-of select="Available" />"
</xsl:if></xsl:template>
