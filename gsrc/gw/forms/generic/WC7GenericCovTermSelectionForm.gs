 ...
 */
@Export
class WC7GenericCovTermSelectionForm extends FormData implements GenericFormInference {
  ...
  final static var COVTERM_TYPES_REQ_VALUES = new ArrayList<IType>() {OptionCovTermPattern.Type, PackageCovTermPattern.Type, TypekeyCovTermPattern.Type}

  override property get DisplayName() : String { 
    return displaykey.Web.Policy.WC7.Forms.WC7GenericCovTermSelectionForm
  }
 ...
 */
  override function getLookupDates(context : FormInferenceContext) : Map<Jurisdiction, DateTime> {     
    ...
    for(cov in _coverables) {
      ...
      if (clause.Pattern.OwningEntityType == "WC7WorkersCompLine") {
  ...
  */
  ...
  function inferredByCoverable(cov : Coverable) : boolean {
    ...
    if(existingCovTerm == null || existingCovTerm.ValueAsString == null || existingCovTerm.ValueAsString == "")
      return false
    if(COVTERM_TYPES_REQ_VALUES.contains(typeof _selectedCovTerm)) { //if this is Option, Package or Typekey covterm
      if(_selectedCovTermValues != null and _selectedCovTermValues.Count  > 0  //values were selected
        ...
        return false  
    } else if(typeof _selectedCovTerm == GenericCovTermPattern.Type and (_selectedCovTerm as GenericCovTermPattern).PropertyType == BooleanCovTerm) {
      if("false".equals(existingCovTerm.ValueAsString.toLowerCase())) {
        return false
      }
    }
    return true
