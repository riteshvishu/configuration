...
class ContractorsEquipmentRatingEngine extends IMAbstractPartRatingEngine {
  ...
  private construct(cePart : ContractorsEquipPart) {
    _part = cePart
    _line = cePart.InlandMarineLine
    _branch = _part.Branch
  }
  
  static function rate(cePart : ContractorsEquipPart) : List<CostData> {
    var engine = new ContractorsEquipmentRatingEngine(cePart)
  ...
  private function rateScheduledEquipment (equipment : ContractorsEquipment) : CostData {
    ...
    var state = _line.BaseState
    var costData = new ContractorsEquipSchedCovCostData(_part.SliceDate, nextSliceDateAfter(_part.SliceDate), RatingCurrency, cov.FixedId, _part.FixedId)
  ...
  private function createEquipPartCovCostData(cov : ContrEquipPartCov) : CostData {
    var costData = new ContractorsEquipPartCovCostData(_part.SliceDate, nextSliceDateAfter(_part.SliceDate), RatingCurrency, cov.FixedId, _part.FixedId)
    costData.NumDaysInRatedTerm = 365
    return costData
  }
