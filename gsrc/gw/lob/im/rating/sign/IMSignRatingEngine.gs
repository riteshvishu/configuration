...
class IMSignRatingEngine extends IMAbstractPartRatingEngine {
  ...
  construct(signPart : IMSignPart) {
    _part = signPart
    _line = signPart.InlandMarineLine
    _branch = _part.Branch
  }
  
  static function rate(signPart : IMSignPart) : List<CostData> {
    var engine = new IMSignRatingEngine(signPart)
  ...
  private function rateSign (sign : IMSign) : IMSignCovCostData {
    ...
    var costData = new IMSignCovCostData(start, end, cov.Currency, cov.FixedId, sign.IMSignPart.FixedId)
