...
enhancement BusinessAutoLineEnhancement : entity.BusinessAutoLine {
   ...
   */
  function getDefaultContainerForAddlInterest() : BusinessVehicle {
  ...
  function syncJurisdictions() {
    var garagingStates = this.Vehicles.map(\ v -> JurisdictionMappingUtil.getJurisdictionMappingForPolicyLocation(v.Location)).toSet()
