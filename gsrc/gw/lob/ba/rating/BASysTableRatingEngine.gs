...
class BASysTableRatingEngine extends AbstractRatingEngine<BusinessAutoLine> {
  ...
  override protected function createCostDataForCost(c : Cost) : CostData {
    switch(typeof c) {
      case BAStateCovVehiclePIPCost: return new BAStateCovVehiclePIPCostData(c)
      case BAStateCovVehicleCost:    return new BAStateCovVehicleCostData(c)
      case BAStateCovCost:           return new BAStateCovCostData(c)
      case BusinessVehicleCovCost:   return new BusinessVehicleCovCostData(c)
      case BALineCovCost:            return new BALineCovCostData(c)
      case BALineCovNonownedCost:    return new BALineCovNonownedCostData(c)
  ...
  private function rateBAOwnedLiabilityCov(cov : BAOwnedLiabilityCov) {
    ...
    for ( vehicle in vehicles )
    {
      ...
      var limitFactor = getRateFactor( "bapLiabLimit", cov.BAOwnedLiabilityLimitTerm, JurisdictionMappingUtil.getJurisdictionMappingForPolicyLocation(vehicle.Location), ratingDate )
      var jur = cov.BALine.getJurisdiction(JurisdictionMappingUtil.getJurisdictionMappingForPolicyLocation(vehicle.Location))
      var cost = new BALineCovCostData(start, end, cov.Currency, jur, cov.FixedId, vehicle.FixedId)
      cost.NumDaysInRatedTerm = this.NumDaysInCoverageRatedTerm
      cost.Basis = 1
      cost.ActualBaseRate = getRateFactor( "bapLiabilityRate", "base", JurisdictionMappingUtil.getJurisdictionMappingForPolicyLocation(vehicle.Location), ratingDate )
  ...
  private function rateBAOwnedMedPayCov(cov : BAOwnedMedPayCov) {
    ...
    for ( vehicle in vehicles )
    {
      ...
      cost.NumDaysInRatedTerm = this.NumDaysInCoverageRatedTerm
      cost.Basis = 1
      cost.ActualBaseRate = getRateFactor( "bapMedPayLimit", cov.BAOwnedMedPayLimitTerm, JurisdictionMappingUtil.getJurisdictionMappingForPolicyLocation(vehicle.Location), ratingDate )
  ...
  private function rateStateCovPerVehicle_impl(cov : BAStateCov, baseRate : BigDecimal, adjRateCalc(baseRate : BigDecimal, v : BusinessVehicle) : BigDecimal) {
    ...
    for ( vehicle in vehicles )
    {
      var cost = new BAStateCovVehicleCostData(start, end, cov.Currency, cov.BAJurisdiction, cov.FixedId, vehicle.FixedId)
      cost.NumDaysInRatedTerm = this.NumDaysInCoverageRatedTerm
      cost.Basis = 1
  ...
  private function rateBAHiredLiabilityCov(cov : BAHiredLiabilityCov) {
    ...
    for (jurisdiction in cov.BALine.HiredAutoJurisdictions) {
      var cost = new BALineCovCostData(start, end, cov.Currency, jurisdiction, cov.FixedId, null)
    ...
    for (jurisdiction in cov.BALine.NonOwnedJurisdictions) {
      ...
      for ( ctype in typekey.BANonOwnedLiabCovCostType.getTypeKeys( false ) ) {
        if ( nonOwnedBasis != null and getBasisByType(nonOwnedBasis, ctype) > 0 ) {
          var cost = new BALineCovNonownedCostData(start, end, cov.Currency, jurisdiction, cov.FixedId, ctype)
          ...
          cost.ActualAdjRate = cost.ActualBaseRate
                          * getUWCompanyRateFactor(jurisdiction.State)
          cost.ActualTermAmount = ( cost.Basis * cost.ActualAdjRate ).setScale( this.RoundingLevel, this.RoundingMode )
          addCost(cost)
        }
      }
  ...
  private function rateStatePIPCov_Impl(cov : BAStateCov, baseRate : BigDecimal,
    ...
    var start = cov.SliceDate
    var end = getNextSliceDateAfter(start)
    var cost = new BAStateCovVehiclePIPCostData(start, end, cov.Currency, cov.BAJurisdiction, cov.FixedId, vehicle.FixedId, costType)
    cost.NumDaysInRatedTerm = this.NumDaysInCoverageRatedTerm
    cost.Basis = 1
    cost.ActualBaseRate = baseRate
    cost.ActualAdjRate = adjRate
  ...
  private function rateBACollisionCov(cov : BACollisionCov) {
    ...
    var baseRate = this.getRateFactor( "bapCollRate", "base",JurisdictionMappingUtil.getJurisdictionMappingForPolicyLocation(cov.Vehicle.Location), ratingDate )
    var deductibleFactor = this.getRateFactor( "bapCollDeductible", cov.BACollisionDeductTerm, JurisdictionMappingUtil.getJurisdictionMappingForPolicyLocation(cov.Vehicle.Location), ratingDate )
    var adjRate = baseRate
                    * deductibleFactor
  ...
  private function rateBAComprehensiveCov(cov : BAComprehensiveCov) {
    var ratingDate = ReferenceDatePlugin.getCoverageReferenceDate(cov.Pattern as CoveragePattern, cov.Vehicle)
    var baseRate = this.getRateFactor( "bapCompRate", "base", JurisdictionMappingUtil.getJurisdictionMappingForPolicyLocation(cov.Vehicle.Location), ratingDate )

    var deductibleFactor = this.getRateFactor( "bapCompDeductible", cov.BAComprehensiveDdctTerm, JurisdictionMappingUtil.getJurisdictionMappingForPolicyLocation(cov.Vehicle.Location), ratingDate )
    var adjRate = baseRate
                    * deductibleFactor
  ...
  private function rateBARentalCov(cov : BARentalCov) {
    var ratingDate = ReferenceDatePlugin.getCoverageReferenceDate(cov.Pattern as CoveragePattern, cov.Vehicle)
    var baseRate = this.getRateFactor( "bapRentalRate", "base", JurisdictionMappingUtil.getJurisdictionMappingForPolicyLocation(cov.Vehicle.Location), ratingDate )
    var adjRate = baseRate
                    * getUWCompanyRateFactor(cov.Vehicle, ratingDate)
    rateVehicleCov_impl( cov, baseRate, adjRate )
  }
  ...
  private function rateBATowingLaborCov(cov : BATowingLaborCov) {
    var ratingDate = ReferenceDatePlugin.getCoverageReferenceDate(cov.Pattern as CoveragePattern, cov.Vehicle)
    var baseRate = this.getRateFactor( "bapTowingRate", "base", JurisdictionMappingUtil.getJurisdictionMappingForPolicyLocation(cov.Vehicle.Location), ratingDate)
    var adjRate = baseRate
                    * getUWCompanyRateFactor(cov.Vehicle, ratingDate)
    rateVehicleCov_impl( cov, baseRate, adjRate )
  }
  ...
  private function rateCancellationShortRatePenalty(lineVersion : BusinessAutoLine) {
    ...
    if ( lineVersion.Branch.RefundCalcMethod == "shortrate" ) { // context.CancelMethod returns null if the period is not canceled
      ...
      for ( st in basesMap.Keys ) {
        ...
        if ( subtotal != 0 ) {
          var cost = new BAJurisdictionCostData(lineVersion.Branch.PeriodStart, lineVersion.Branch.PeriodEnd, lineVersion.Branch.PreferredSettlementCurrency, "CancelShortRatePenalty",
  ...
  private function rateMinimumPremium(lineVersion : BusinessAutoLine) {
    if (lineVersion.Branch.RefundCalcMethod != "flat" ) {  // context.CancelMethod returns null if the period is not canceled
      ...
      if ( subtotal < minPremium ) { 
        ...
        var cost = new BAMinimumPremiumCostData(lineVersion.Branch.PeriodStart, lineVersion.Branch.PeriodEnd, lineVersion.Branch.PreferredSettlementCurrency, jur)
        cost.NumDaysInRatedTerm = lineVersion.Branch.NumDaysInPeriod
        cost.Basis              = subtotal
  ...
  private function rateStateTaxes(lineVersion : BusinessAutoLine) {
    ...
    for ( st in basesMap.Keys ) {
      ...
      if ( subtotal != 0 ) {
        ...
        var cost = new BAJurisdictionCostData(lineVersion.Branch.PeriodStart, lineVersion.Branch.PeriodEnd, TaxRatingCurrency, "StateTax",
  ...
  private function getAmountSumByState(cDatas : List<BACostData>) : Map<Jurisdiction, BigDecimal> {
    return cDatas.partition( \ c -> c.JurisdictionArg.State ).mapValues( \ s -> s.sum(\c -> c.ActualAmount) ).toAutoMap( \ s -> BigDecimal.ZERO )
  }

  private function rateVehicleCov_impl(cov : BusinessVehicleCov, baseRate : BigDecimal, adjRate : BigDecimal) {
    var start = cov.SliceDate
    var end = getNextSliceDateAfter(start)
    var jur = cov.Vehicle.BALine.getJurisdiction(JurisdictionMappingUtil.getJurisdictionMappingForPolicyLocation(cov.Vehicle.Location))
    var cost = new BusinessVehicleCovCostData(start, end, cov.Currency, jur, cov.FixedId, cov.Vehicle.FixedId)
//    cost.RatedOrder         = "CoveragePremium" // AHK - This is currently set when the cost is created; maybe it should live on BACostData instead
  ...
  private function getRadiusFactor(vehicle : BusinessVehicle, ratingDate : Date) : BigDecimal
  {
    return new RateAdjFactorSearchCriteria( "bapVehicleRadius", ratingDate )
             .match( vehicle.VehicleRadius.Code, JurisdictionMappingUtil.getJurisdictionMappingForPolicyLocation(vehicle.Location ))
  }
  ...
  {
    if (isFactorTrue) {
      return new RateAdjFactorSearchCriteria( factorName, ratingDate ).match( JurisdictionMappingUtil.getJurisdictionMappingForPolicyLocation(vehicle.Location ))
    }
    return 1.0
  }
  ...
  private function getAgeFactor(vehicle : BusinessVehicle, ratingDate : Date) : BigDecimal
  {
    ...
    return new RateAdjFactorSearchCriteria( "bapVehicleAge", ratingDate ).matchInRange( age, JurisdictionMappingUtil.getJurisdictionMappingForPolicyLocation(vehicle.Location) )
  }

  private function getCostNewFactor(vehicle : BusinessVehicle, ratingDate : Date) : BigDecimal
  {
    return new RateAdjFactorSearchCriteria( "bapVehicleCostNew", ratingDate ).matchInRange( vehicle.CostNew as Number, JurisdictionMappingUtil.getJurisdictionMappingForPolicyLocation(vehicle.Location) )
  }

  private function getUWCompanyRateFactor(vehicle : BusinessVehicle, ratingDate : Date) : BigDecimal
  {
    return vehicle.BALine.Branch.getUWCompanyRateFactor( ratingDate, JurisdictionMappingUtil.getJurisdictionMappingForPolicyLocation(vehicle.Location) )
  }
}
