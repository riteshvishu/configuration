...
class CPSysTableRatingEngine extends AbstractRatingEngine<CommercialPropertyLine> {
  ...
  override protected function createCostDataForCost(c : Cost) : CostData {
    switch(typeof c) {
      case CPBuildingCovBroadCost: return new CPBuildingCovBroadCostData(c)
      case CPBuildingCovGrp1Cost:  return new CPBuildingCovGroup1CostData(c)
      case CPBuildingCovGrp2Cost:  return new CPBuildingCovGroup2CostData(c)
      case CPBuildingCovSpecCost:  return new CPBuildingCovSpecialCostData(c)
  ...
  function rateCPBldgCov(cov : CPBldgCov) : List<CPBuildingCovCostData> {
    ...
    var state = JurisdictionMappingUtil.getJurisdictionMappingForPolicyLocation(cov.CPBuilding.CPLocation.Location )

    costs.add(rateCPBuildingCov_impl(new CPBuildingCovGroup1CostData(start, end, cov.Currency, cov.FixedId, state), cov, 0.12, limitValue, deductibleValue, "cpDeductGrp1"))
    costs.add(rateCPBuildingCov_impl(new CPBuildingCovGroup2CostData(start, end, cov.Currency, cov.FixedId, state), cov, 0.08, limitValue, deductibleValue, "cpDeductGrp2"))
    if (cov.CPBldgCovCauseOfLossTerm.Value == "Broad" or cov.CPBldgCovCauseOfLossTerm.Value == "Special") {
      costs.add(rateCPBuildingCov_impl(new CPBuildingCovBroadCostData(start, end, cov.Currency, cov.FixedId, state), cov, 0.08, limitValue, deductibleValue, "cpDeductBroad"))
    }
    if (cov.CPBldgCovCauseOfLossTerm.Value == "Special") {
      costs.add(rateCPBuildingCov_impl(new CPBuildingCovSpecialCostData(start, end, cov.Currency, cov.FixedId, state), cov, 0.08, limitValue, deductibleValue, "cpDeductSpecial"))
    }
    return costs
  }
  ...
  function rateCPBPPCov(cov : CPBPPCov) : List<CPBuildingCovCostData> {
    ...
    costs.add(rateCPBuildingCov_impl(new CPBuildingCovGroup1CostData(start, end, cov.Currency, cov.FixedId, state),cov, 0.15, limitValue, deductibleValue, "cpDeductGrp1"))
    costs.add(rateCPBuildingCov_impl(new CPBuildingCovGroup2CostData(start, end, cov.Currency, cov.FixedId, state), cov, 0.15, limitValue, deductibleValue, "cpDeductGrp2"))
  ...
  function rateCPBuildingCov_impl(costData : CPBuildingCovCostData, cov : CPBuildingCov, stdBaseRate : BigDecimal,
    ...
    costData.StandardBaseRate     = stdBaseRate
    ...
    costData.StandardAdjRate      = costData.StandardBaseRate * deductibleFactor * fireProtectionClassFactor * uwCompanyFactor
                                    * Branch.getProductModifierFactor()

    var basisFactor = 0.01  // CP rates per $100 of limit
    costData.StandardTermAmount = (costData.Basis * costData.StandardAdjRate * basisFactor)
  ...
  protected override function rateWindow(lineVersion : CommercialPropertyLine) {
    ...
    var basesMap  = cpBuildingCovCostDatas.partition(\ c -> c.State).mapValues(\ l -> l.sum(\ c -> c.ActualAmount))
    for (st in basesMap.Keys) {
      var subtotal = basesMap.get(st)
      if (subtotal != 0) {
        var costData = new CPStateTaxCostData(lineVersion.Branch.PeriodStart, lineVersion.Branch.PeriodEnd, TaxRatingCurrency, st)
