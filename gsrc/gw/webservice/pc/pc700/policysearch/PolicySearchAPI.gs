...
uses java.util.Date
uses com.guidewire.commons.system.exception.MultipleMatchesException
...
class PolicySearchAPI {
  ...
  @Throws(SOAPException, "If communication errors occur")
  @Throws(RequiredFieldException, "If required field is missing")
  @Throws(MultipleMatchesException, "If there are more than one polcy by this Policy number.")
  ...
  function findPolicyPublicIdByPolicyNumber(policyNumber: String): String {
    ...
    if (policyNumber == Plugins.get(IPolicyPeriodPlugin).UnassignedPolicyNumberIdentifier) {
      throw new IllegalArgumentException("Unassigned policy numbers can't be uniquely mapped to PolicyPublicId")
    }
