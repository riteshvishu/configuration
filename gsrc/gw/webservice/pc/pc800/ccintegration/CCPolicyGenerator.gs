...
class CCPolicyGenerator {
  ...
  public function generatePolicySummary( pcSummary : PolicyPeriodSummary, asOfDate : Date ) : CCPolicySummary {
    ...
    if (!pcSummary.TermArchived) {
      ...
      for (wrk in vehicleList) {
        summ.Vehicles.add(new CCPolicySummary_Vehicles(wrk))
      }
      for (wrk in propertyList) {
       summ.Properties.add(new CCPolicySummary_Properties(wrk))
      }
    }
  ...
  public function addAddress(id : Key, ti : CCAddress) : XmlElement {
    ti.ID = "_" + _mappedPCObjects.Count;
    var el = new Envelope_CCAddress(ti)
...
//    print("does not meetsPolicySystemIDFilteringCriteria(id=" + id)
  ...
  public function meetsDriverFilteringCriteria(lastName : String) : boolean {
    ...
    if (_filter.DriverLastNames.Entry.Empty) return false  // nothing in the list, so cannot match

    for (crit in _filter.DriverLastNames.Entry) {
