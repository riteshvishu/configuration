...
class ProducerAPI {
  ...
  function updateOrganization(orgModel : OrganizationDTO) {
    ...
    gw.transaction.Transaction.runWithNewBundle(\ bundle -> {
      var orgInBundle = bundle.loadByKey(foundOrg.ID) as Organization
  ...
  function updateGroup(groupModel : GroupDTO) {
    ...
    gw.transaction.Transaction.runWithNewBundle(\ bundle -> {
      var groupInBundle = bundle.loadByKey(foundGroup.ID) as Group
  ...
  function updateProducerCode(producerCodeModel : ProducerCodeDTO) {
    ...
    gw.transaction.Transaction.runWithNewBundle(\ bundle -> {
      var producerCodeInBundle = bundle.loadByKey(foundGroup.ID) as ProducerCode
  ...
  private function loadOrgByPublicID(publicID : String) : entity.Organization {
    return Query.make(Organization).compare("PublicID", Equals, publicID).select().AtMostOneRow
  }
  
  private function loadGroupByPublicID(publicID : String) : entity.Group {
    return Query.make(Group).compare("PublicID", Equals, publicID).select().AtMostOneRow
  }
  
  private function loadProducerCodeByPublicID(publicID : String) : entity.ProducerCode {
    return Query.make(ProducerCode).compare("PublicID", Equals, publicID).select().AtMostOneRow
  }  
}
