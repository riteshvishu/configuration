...
enhancement PolicyAssignmentEnhancement : entity.Policy
{
  ...
  function getSuggestedPreRenewalOwners(direction : PreRenewalDirection) : Assignee[] {
    ...
    goodRoleSet.add( Role("underwriter_supervisor"))
    ...
    switch (direction) {
      case "custrep": 
        ...
        // fall through
      case "assistant": 
        goodRoleSet.add( Role("underwriter_asst"))
      ...
      case "underwriter": 
      case "nonrenewrefer":  
        goodRoleSet.add( Role("underwriter")) // fall through
  ...
  function createRoleActivity( role : typekey.UserRole, pattern : ActivityPattern, subject : String, 
    ...
    if (roleAssignment != null) {
      if (not activity.assign( roleAssignment.AssignedGroup, roleAssignment.AssignedUser )) {
        Logger.logWarning(role + " was unable to be assigned to activity: " + roleAssignment.AssignedUser +
