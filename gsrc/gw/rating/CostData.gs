...
abstract class CostData<S extends entity.Cost, V extends PolicyLine> extends CostDataBase implements WorksheetEntryContainer{
  ...
  construct(effDate : DateTime, expDate : DateTime) {
    ...
    initializeCostData(effDate, expDate, CurrencyUtil.getDefaultCurrency())
  }
   ...
   */
  construct(effDate : DateTime, expDate : DateTime, asRatedCurrency : Currency) {
    initializeCostData(effDate, expDate, asRatedCurrency)
  }
   ...
   */
  private function initializeCostData(effDate : DateTime, expDate : DateTime, asRatedCurrency : Currency) {
   ...
   */
  abstract protected function getVersionedCosts(line : V) : List<com.guidewire.commons.entity.effdate.EffDatedVersionList>
  ...
  function copyOverridesFromCost(c : Cost) {
    if (c != null and not c.CurrencyChanged) {
  ...
  protected function populateRatingWorksheet(line : V, cost : Cost){
    if (Plugins.get(IRateRoutinePlugin).worksheetsEnabledForLine(line.PatternCode)) {
      if (_entries.Count > 0) {
        if (cost.Worksheet == null) {
          cost.initializeWorksheet()          
        }
        var worksheet = new Worksheet() {:Description = "${cost}", :WorksheetEntries = _entries }
        cost.Worksheet.TextData = worksheet.toXml().asUTFString()
      }
    } else if (cost.Worksheet != null ){
      (cost.Worksheet as KeyableBean).remove()
    }
  }
  ...
  */
  ...
  protected function isCostEqual(other : CostData) : boolean {
    switch(false){
      case BigDecimalUtil.isEqual(Basis, other.Basis): logAlmostMergedCost(other, "Basis"); return false
      case BigDecimalUtil.isEqual(ActualBaseRate, other.ActualBaseRate): logAlmostMergedCost(other, "ActualBaseRate"); return false
      case BigDecimalUtil.isEqual(ActualAdjRate, other.ActualAdjRate) : logAlmostMergedCost(other, "ActualAdjRate"); return false
      case BigDecimalUtil.isEqual(ActualTermAmount, other.ActualTermAmount) : logAlmostMergedCost(other, "ActualTermAmount"); return false
      case NumDaysInRatedTerm == other.NumDaysInRatedTerm: logAlmostMergedCost(other, "NumDaysInRatedTerm"); return false
      case ActualAmount == null : logAlmostMergedCost(other, "ActualAmount"); return false
      case other.ActualAmount == null : logAlmostMergedCost(other, "ActualAmount"); return false
      case BigDecimalUtil.isEqual(OverrideBaseRate, other.OverrideBaseRate) : logAlmostMergedCost(other, "OverrideBaseRate"); return false
      case BigDecimalUtil.isEqual(OverrideAdjRate, other.OverrideAdjRate) : logAlmostMergedCost(other, "OverrideAdjRate"); return false
      case BigDecimalUtil.isEqual(OverrideTermAmount, other.OverrideTermAmount) : logAlmostMergedCost(other, "OverrideTermAmount"); return false
      default: return true
    }
  }
  ...
  public function isBasisScalableCostEqual(other : CostData) : boolean {
    switch(false){
      ...
      case !(OverrideTermAmount != null and OverrideAmount == null and other.OverrideTermAmount == null and other.OverrideAmount != null):
        logAlmostMergedCost(other, "OverrideTermAmount and OverrideAmount"); return false
      case !(OverrideTermAmount == null and OverrideAmount != null and other.OverrideTermAmount != null and other.OverrideAmount == null):
        logAlmostMergedCost(other, "OverrideTermAmount and OverrideAmount"); return false
      default: return true
    }
   ...
   */
  private function findExistingCost(costVL : com.guidewire.commons.entity.effdate.EffDatedVersionList) : S {
    var cost : S = costVL.getVersionAsOf(EffectiveDate) as S  // get the cost at the slice date
