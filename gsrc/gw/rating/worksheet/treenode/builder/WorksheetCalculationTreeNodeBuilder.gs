...
class WorksheetCalculationTreeNodeBuilder extends WorksheetTreeNodeBuilder<WorksheetCalculation> {
  ...
  private function getRoundingModeTypeFromNameLocaleAgnostic(operandName : String) : RoundingModeType {
    var result : RoundingModeType = null
    var allLocales = PLDependencies.getLanguages().getAllLocales()

    for (locale in allLocales) {
      LocaleUtil.runAsCurrentLanguage(locale, \ -> {
        result = RoundingModeType.getTypeKeys().singleWhere( \ roundingMode -> operandName.equals(roundingMode.getDisplayName(locale)))
      })
    }
