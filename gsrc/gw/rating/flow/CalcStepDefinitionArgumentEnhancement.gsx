...
enhancement CalcStepDefinitionArgumentEnhancement : entity.CalcStepDefinitionArgument {
  ...
  private function getTargetParamType() : IType {
    if (this.Operand.OperandType != CalcStepOperandType.TC_RATEFUNC) {
      throw new IllegalStateException("Attemped to get function target param type from non rate function operand")
    }
  ...
  property get ArgumentSourceFromTableDefs() : entity.RateTableArgumentSource {
    ...
    if (matchOp != null and matchOp.ArgumentSources.Count > 0) {
      ...
      if (argSrcSetCode.HasContent) {
        ...
        RateFlowLogger.get().warn("Rate table operand does not have an argument source set code.")
      }
    }
    return null
  }
  ...
  property get ArgumentValueForDependency() : String {
    switch (this.OperandType) {
      ...
      case TC_INSCOPE:
        ...
        } else if (this.InScopeValueType.equals("entity.Coverage")) {
  ...
  function getDependentList(): List<RateTableColumn> {
