...
uses wsi.remote.gw.webservice.ab.ab800.abcontactapi.types.complex.ABContactAPISearchResult
...
class DemoContactSystemPlugin implements ContactSystemPlugin {
  ...
  private static var _mapper = ContactIntegrationMapperFactory.get()
  ...
  override function retrieveContact(contactUid: String, creator : ContactCreator) : Contact {
    ...
    if (mockContact == null) {
      throw new DisplayableException("Could not find contact with id of ${contactUid}")
    }
  ...
  private function createSoapAddress(i : String) : XmlBackedInstance {
    ...
    var nameMapper = ContactIntegrationMapperFactory.get().getNameMapper()
