...
uses wsi.remote.gw.webservice.bc.bc800.paymentinstrumentapi.PaymentInstrumentAPI
...
class BCBillingSystemPlugin implements IBillingSystemPlugin {
  ...
  construct() {
    _logger = LoggerFactory.getLogger("BillingIntegration")
  }
  ...
  private property get PaymentAPIWithLanguage() : PaymentInstrumentAPI {
    var paymentAPI = new PaymentInstrumentAPI()
    paymentAPI.Config.Guidewire.Locale = LocaleUtil.CurrentLanguage.Code
    return paymentAPI
  }
  ...
  override function createAccount(account : Account, transactionID : String) : String {
    ...
    return callUpdate(\ b -> BillingAPIWithLanguage.createAccount(accountInfo, transactionID))
  }
  ...
  override function retrieveAllPaymentPlans(policyPeriod : PolicyPeriod) : PaymentPlanSummary[] {
    ...
    if (policyPeriod.BillingMethod == BillingMethod.TC_LISTBILL){
      ...
      bcPaymentPlans = BillingAPIWithLanguage.getAllPaymentPlans()
    }
  ...
  override function getAvailableBillingMethods(producerCode : String) : BillingMethod[] {
    var billingMethods = BillingAPIWithLanguage.getAvailableBillingMethods(producerCode)
  ...
  */
  ...
  override function retrieveAllAgencyBillPlans() : AgencyBillPlanSummary[] {
    ...
    var plans = BillingAPIWithLanguage.getAllAgencyBillPlans()
  ...
  override function retrieveAllCommissionPlans() : CommissionPlanSummary[] {
    ...
    var plans = BillingAPIWithLanguage.getAllCommissionPlans()
  ...
  override function getSubAccounts(accountNumber : String) : BillingAccountSearchResult[] {
    ...
    try {
      var bcResults = BillingAPIWithLanguage.getAllSubAccounts(accountNumber)
  ...
  override function getInvoiceStreams(accountNumber : String) : BillingInvoiceStreamInfo[] {
    ...
    var infos = BillingAPIWithLanguage.getAccountInvoiceStreams(accountNumber)
  ...
  override function getExistingPaymentInstruments(accountNumber : String) : BillingPaymentInstrument[] {
    try{
      var instruments = PaymentAPIWithLanguage.getPaymentInstrumentsForAccount(accountNumber)
      return instruments
        .where(\ p -> p.PaymentMethod <> wsi.remote.gw.webservice.bc.bc800.paymentinstrumentapi.enums.PaymentMethod.Responsive) // filter out responsive
        .map(\ p -> createBillingPaymentInstrument(p))
    }catch(e : wsi.remote.gw.webservice.bc.bc800.paymentinstrumentapi.faults.BadIdentifierException){
  ...
  override function addPaymentInstrumentTo(accountNumber : String, paymentInstrument : BillingPaymentInstrumentJava) : BillingPaymentInstrumentJava {
    ...
    var bcInstrument = new wsi.remote.gw.webservice.bc.bc800.paymentinstrumentapi.types.complex.PaymentInstrumentRecord(){
      :PaymentMethod = wsi.remote.gw.webservice.bc.bc800.paymentinstrumentapi.enums.PaymentMethod.forGosuValue(instrument.PaymentMethod.Code),
      :OneTime = false,
      :Token = instrument.Token
    }
    bcInstrument = PaymentAPIWithLanguage.createPaymentInstrumentOnAccount(accountNumber, bcInstrument)
  ...
  private function createBillingPaymentInstrument(p : wsi.remote.gw.webservice.bc.bc800.paymentinstrumentapi.types.complex.PaymentInstrumentRecord) : BillingPaymentInstrument{
  ...
  override function retrieveAccountUnappliedFunds(accountNumber : String) : BillingUnappliedFundInfo[] {
    try{
      var unappliedFundInfos = BillingAPIWithLanguage.getUnapplieds(accountNumber)
      return unappliedFundInfos.map(\ info -> new BCUnappliedFundWrapper(info))
    }catch(e : wsi.remote.gw.webservice.bc.bc800.billingapi.faults.BadIdentifierException) {
      // account is new, just ignore the exception and return empty list
      _logger.info("Trying to get unapplied funds for account that not exist in BC yet")
      return {}
    }
  }
