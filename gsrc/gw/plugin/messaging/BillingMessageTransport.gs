...
class BillingMessageTransport implements MessageTransport {
    ...
    try {
      switch(message.EventName){
        ...
        case RENEWPERIOD_MSG:
          ...
          createAccountIfNecessary(policyPeriod.Policy.Account, getTransactionId(message)+ "-1")
          plugin.renewPolicyPeriod(policyPeriod, getTransactionId(message))
          break
        case REWRITEPERIOD_MSG:
          if(not (policyPeriod.Job typeis Rewrite)){
            throw new IllegalStateException("Unexpected job type ${policyPeriod.Job.Subtype} for message ${message.EventName}")
          }
          plugin.rewritePolicyPeriod(policyPeriod, getTransactionId(message))
          break
        case FINALAUDIT_MSG:
  ...
  private function shouldSendContactUpdate(contact : Contact) : boolean {
    ...
    var plugin = Plugins.get(IBillingSystemPlugin)
    if(plugin.CompatibilityMode.equals("4.0.0")){
      // only send account contact update for older version of the integration
      return contact.isPolicyContact() or not contact.findHeldAccounts().Empty
    }
    // from 7.0.0, we don't send contact update for account & policy to Billing System
    // anymore but instead send to Contact System.
    return false
  }

  private function createAccountIfNecessary(account : Account, transactionId : String) {
    var plugin = Plugins.get(IBillingSystemPlugin)
    if (not plugin.isAccountExist(account.AccountNumber)) {
  ...
  private function createAltBillingAccountIfNecessary(accountNumber : String, transactionId : String) {
    var plugin = Plugins.get(IBillingSystemPlugin)
    if (accountNumber <> null and not plugin.isAccountExist(accountNumber)) {
  ...
  private function createProducer(organization : Organization, transactionId : String) {
    ...
    var plugin = Plugins.get(IBillingSystemPlugin)
    if (organization.Producer and not plugin.isProducerExist(organization.PublicID)) {
