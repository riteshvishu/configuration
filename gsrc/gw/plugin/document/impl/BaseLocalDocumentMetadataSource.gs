...
abstract class BaseLocalDocumentMetadataSource
  ...
  */
  ...
  protected property get MetadataDir() : File {
    return DocumentConfigUtil.getMetadataDir().toJavaFile()
  }
  ...
  */  
  private function retrieveDocument( metadataFile : File, doThrow : boolean) : Document {
    ...
    try {
      ...
      Logger.DOCUMENT.info("BaseLocalDocumentMetadataSource retrieveDocument id=${result.ID} docUID=${result.DocUID} publicId=${result.PublicId}" )
      return result
    } 
    catch (e : Exception) {
      if (doThrow) {
      ...
      if (xml != null) {
        try {
          ...
          // ignore
          Logger.DOCUMENT.debug(e.Message, e)
        }
      }
    }
  }
  ...
  */
  protected function storeDocument(document : Document) {
    var metadataFile = getMetadataFileForUniqueId(document.PublicID)
    Logger.DOCUMENT.info( "BaseLocalDocumentMetadataSource.store id=${document.Id} docUUD=${document.DocUID} file=${metadataFile}" )
