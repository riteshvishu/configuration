...
/** --- COMMENTED OUT AS THESE WILL NOT WORK IN CUSTOMER ONLY MODE --- */
 ...
 private function changeEEDAndNameOnPNI(testName : String, basedOn : PolicyPeriod) {
   ...
   pChange.PolicyChangeProcess.changeEditEffectiveDate(iDay.addMonths(2))
 ...
 private function createPreemptionSameDate(testName : String, basedOn : PolicyPeriod) {
  ...
  override function load() {
    ...
    var config = com.guidewire.pl.system.dependency.PLDependencies.getPluginConfig()
    var existingPluginDef = config.getPluginDef(IPolicyNumGenPlugin)
    try {
      var newPluginDef = new com.guidewire.pl.system.integration.plugins.PluginDefMock(IPolicyNumGenPlugin, new AccountBasedPolicyNumPlugin())
          ...
          runBlockAsUser(underwriter, "SearchSamplePolicyData -- ${underwriter}", \ -> {
            ...
            if (underwriter == "aapplegate") {
              if (i % 4 != 0) {
                ...
                loadSubmission(jobNum, account, "PersonalAuto", SampleDataConstants.getBaseDateMinus(90), namedInsureds, true, addIDs("solr:" + jobNum))
              } else {
                loadSubmission(jobNum, account, "BusinessOwners", SampleDataConstants.getBaseDateMinus(275), namedInsureds, true, addIDs("solr:" + jobNum))
              }
            } else {
              switch (i % 4) {
                case 0:
                case 1:
                  loadSubmission(jobNum, account, "BusinessOwners", SampleDataConstants.getBaseDateMinus(120), namedInsureds, true, addIDs("solr:" + jobNum))
                  break
                case 2:
                  loadSubmission(jobNum, account, "BusinessAuto", SampleDataConstants.getBaseDateMinus(150), namedInsureds, true, addIDs("solr:" + jobNum))
                  break
                case 3:
                  loadSubmission(jobNum, account, "WorkersComp", SampleDataConstants.getBaseDateMinus(210), namedInsureds, true, addIDs("solr:" + jobNum))
