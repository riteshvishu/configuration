...
abstract class AbstractPolicyLineMethodsImpl implements PolicyLineMethods {
  ...
  override property get AllClauses() : Clause[] {
    ...
    AllCoverages.each( \ cov -> clauses.add(cov as Clause) )
    AllExclusions.each( \ excl -> clauses.add(excl as Clause) )
    AllConditions.each( \ cond -> clauses.add(cond as Clause) )
  ...
  override function getWorksheetRootNode(showConditionals : boolean) : RowTreeRootNode {
    var premiums = _line.Costs.Premiums.orderBy(\ c -> c.DisplayName).toList()
    var taxes = _line.Costs.TaxSurcharges.orderBy(\ c -> c.DisplayName).toList()
    var allCosts : List<Cost> = {}
    allCosts.addAll(premiums)
    allCosts.addAll(taxes)

    return WorksheetTreeNodeUtil.buildRootNode(allCosts, showConditionals)
  }
